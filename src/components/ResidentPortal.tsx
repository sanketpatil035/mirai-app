...
export function ResidentPortal({
  ...
  // Selected Booking details for QR Modal
  const [selectedBookingForQR, setSelectedBookingForQR] = useState<AmenityBooking | null>(null);
  ...
  return (
      <div className="space-y-8 animate-fade-in" id="resident-portal-root">
                              ...
                              <div className="pt-2 border-t border-slate-50 flex items-center justify-between text-[10px] text-slate-400 font-mono">
                                ...
                                {b.status === "Approved" && (
                                    <button
                                        ...
                                        className="bg-teal-50 text-teal-700 hover:bg-teal-600 hover:text-white border border-teal-200/60 rounded-lg px-2 py-1 text-[9px] font-bold font-sans uppercase tracking-wider flex items-center gap-1.5 transition-all shadow-xs cursor-pointer active:scale-95"
                                        title="Click to view secure gate entry QR pass"
                                    >
        ...
        {/* Interactive, high-fidelity security barcode & QR pass modal */}
        {selectedBookingForQR && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" id="qrcode-pass-modal">
              <div className="relative w-full max-w-sm bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-150 animate-fade-in flex flex-col">
                {/* Artistic Security Header */}
                <div className="bg-slate-900 text-white px-6 py-5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="h-7 w-7 bg-teal-500 rounded-lg flex items-center justify-center text-slate-950 font-bold">
                      <QrCode className="h-4 w-4" />
                    </div>
                    <div>
                      <h4 className="text-xs font-bold tracking-tight uppercase">Smart Gate Pass</h4>
                      <p className="text-[9px] font-mono tracking-widest text-teal-400">GK MIRAI PRE-APPROVED</p>
                    </div>
                  </div>
                  <button
                ...
                {/* Ticket body details */}
                <div className="p-6 space-y-6 flex-1 bg-gradient-to-b from-teal-50/20 via-white to-white select-none">

                  {/* QR Scanner visual field */}
                  <div className="relative flex flex-col items-center justify-center pt-2">
                    <div className="relative border-4 border-slate-900 bg-white p-5 rounded-2xl shadow-md">
                      {/* Decorative corner brackets of a scanning camera */}
                      <div className="absolute top-2 left-2 h-4 w-4 border-t-2 border-l-2 border-teal-500 rounded-tl-xs"></div>
                      <div className="absolute top-2 right-2 h-4 w-4 border-t-2 border-r-2 border-teal-500 rounded-tr-xs"></div>
                      <div className="absolute bottom-2 left-2 h-4 w-4 border-b-2 border-l-2 border-teal-500 rounded-bl-xs"></div>
                      <div className="absolute bottom-2 right-2 h-4 w-4 border-b-2 border-r-2 border-teal-500 rounded-br-xs"></div>

                      {/* QR Code central vector */}
                      <QrCode className="h-36 w-36 text-slate-900" />

                      {/* Laserscan blinking feedback bar */}
                      <span className="absolute inset-x-4 h-0.5 bg-emerald-500 shadow-[0_0_12px_2.5px_rgba(16,185,129,0.8)] animate-bounce animate-duration-2000"></span>
                    </div>

                    <span className="mt-4 text-[10px] font-mono font-extrabold tracking-widest text-slate-500 uppercase bg-slate-100 px-3 py-1 rounded-full">
                  TOKEN: GKM-{selectedBookingForQR.id.substring(5, 12).toUpperCase()}
                </span>
                  </div>

                  {/* Passenger and ticket credentials info */}
                  <div className="border-t border-dashed border-slate-200 pt-5 space-y-4">

                    {/* Visual cutout circles to make it look like a physical ticket */}
                    <div className="absolute left-[-10px] top-[195px] h-5 w-5 rounded-full bg-slate-900/60 backdrop-blur-md"></div>
                    <div className="absolute right-[-10px] top-[195px] h-5 w-5 rounded-full bg-slate-900/60 backdrop-blur-md"></div>

                    <div className="grid grid-cols-2 gap-y-3.5 gap-x-2 text-xs">
                      <div>
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Authorized Visitor</span>
                        <span className="font-sans font-bold text-slate-800 leading-tight">{activeUnit.ownerName}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Assigned Unit No</span>
                        <span className="font-mono font-bold text-teal-600">Unit {activeUnit.flatNo}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Booked Facility</span>
                        <span className="font-sans font-bold text-slate-800 leading-tight">{selectedBookingForQR.amenityName}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Total Admitted</span>
                        <span className="font-sans font-bold text-slate-800">{selectedBookingForQR.guestsCount} Persons Max</span>
                      </div>
                      <div className="col-span-2 border-t border-slate-100 pt-3 flex justify-between items-center text-xs">
                        <div>
                          <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Scheduled Slot Date & Hour</span>
                          <span className="font-sans font-semibold text-slate-800">{selectedBookingForQR.date}</span>
                          <span className="block font-mono text-[10px] text-teal-600 font-semibold">{selectedBookingForQR.timeSlot}</span>
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Barcode representation at ticket footer */}
                  <div className="pt-4 border-t border-slate-100 flex flex-col items-center gap-1">
                    <div className="h-6 w-full flex items-center justify-between opacity-70">
                      {Array.from({ length: 42 }).map((_, idx) => (
                          <span
                              key={idx}
                              className="bg-slate-950 h-full inline-block"
                              style={{
                                width: `${[1, 2, 3, 1, 4, 1, 2, 1, 3][idx % 9]}px`
                              }}
                          ></span>
                      ))}
                    </div>
                    <span className="text-[9px] font-mono tracking-widest text-slate-400 uppercase">MyGate Smart Pass Verification</span>
                  </div>

                </div>

                {/* Quick Action bar (Print/Export) */}
                <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex gap-3">
                  <button
                      onClick={() => {
                        window.print();
                      }}
                      className="flex-1 py-2.5 rounded-xl border border-slate-200 hover:border-slate-350 hover:bg-white text-slate-700 font-semibold text-xs tracking-wide transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer bg-slate-50 text-center"
                  >
                  ...
                  <button
                      onClick={() => {
                        alert("Gate entry credentials downloaded securely! Pre-auth synced with automated gate guard terminal.");
                        setSelectedBookingForQR(null);
                      }}
                      className="flex-1 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs tracking-wide transition-all shadow-md shadow-teal-600/10 flex items-center justify-center gap-2 cursor-pointer text-center"
                  >
